...
const VS = {
  // Boden
  grass: '#6f8a45', grassD: '#57703a', grassL: '#89a45c', grassEdge: '#3e5227',
  soil: '#7c5a38', soilD: '#4c3820',
  cobble: '#928c7e', cobbleD: '#6d685c', cobbleL: '#aca595',
  path: '#8b7350', pathD: '#6a563a',
  // Holz / Fachwerk
  wood: '#7d5e3c', woodD: '#4f3922', woodL: '#9a7a4e',
  plaster: '#d3c6aa', plasterD: '#b7a888', plasterL: '#e6dcc4',
  // Stein
  stone: '#8b919d', stoneD: '#595f6b', stoneL: '#aab0bb',
  mortar: '#6b7078',
  // Dächer
  shingle: '#6e4a30', shingleD: '#472e1c', shingleL: '#8a5f3e',
  slate: '#4f5866', slateD: '#343b46', slateL: '#6d7684',
  thatch: '#b19a63', thatchD: '#897548', thatchL: '#cab577',
  // Diverses
  win: '#ffd98a',
  leaf: '#4d7a38', leafD: '#2e4b20', leafL: '#6f9c4a', pine: '#3f6330', pineD: '#274018',
  water: '#3f7fa8', waterD: '#2a597a', waterL: '#7fb4d6',
  dark: '#241d17', cloth1: '#9c4a3f', cloth1L: '#b5675a', cloth2: '#4a6f7c', cloth2L: '#628f9c',
};

// ---------- Iso-Geometrie ----------
const HW = 80, HH = 40;          // halbe Kachel (Vollkachel 160×80, Verhältnis 2:1)
const OX = 800, OY = 250;        // Bildschirm-Ursprung (Kachel 0,0)
const RIM = 26;                  // Höhe des Erdrands der Insel
const LO = -2, HI = 6;           // Kachelbereich der Insel (9×9)
const VBW = 1600, VBH = 880;     // viewBox-Maße
...
const GRID = {
  eisen: [0, 0], stein: [2, 0], kaserne: [4, 0],
  holz: [0, 2], rathaus: [2, 2], farm: [4, 2],
  markt: [0, 4], lager: [2, 4], mauer: [4, 4],
};
...
const P = (arr, f, extra = '') => `<polygon points="${pts(arr)}" fill="${f}"${extra}/>`;
const C = (x, y, r, f, extra = '') => `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}" fill="${f}"${extra}/>`;
const L = (a, b, col, w = 2) => `<line x1="${a[0].toFixed(1)}" y1="${a[1].toFixed(1)}" x2="${b[0].toFixed(1)}" y2="${b[1].toFixed(1)}" stroke="${col}" stroke-width="${w}"/>`;
...
function prism(sc, sr, h, top, left, right, yb = 0) {
  ...
  const ft = up(b.front, h), rt = up(b.right, h), bt = up(b.back, h), lt = up(b.left, h);
  return (
...
function roofSh(sc, sr, rh, cl, cr, yb = 0) {
  ...
  for (let t = 0.2; t < 0.92; t += 0.19) {
    const lp = lerp(b.left, apex, t), fp = lerp(b.front, apex, t), rp = lerp(b.right, apex, t);
...
function frame(a, b, h, col = VS.woodD) {
  const at = up(a, h), bt = up(b, h);
  const mb = mid(a, b), mt = mid(at, bt);
  const rail = [lerp(a, at, 0.5), lerp(b, bt, 0.5)];
  return (
    L(a, at, col, 2.6) + L(b, bt, col, 2.6) +      // Eckpfosten
    L(mb, mt, col, 2.2) +                          // Mittelpfosten
    L(rail[0], rail[1], col, 2.2) +                // Querriegel
    L(a, mt, col, 1.8) + L(b, mt, col, 1.8)        // Andreaskreuz-Streben
  );
}
...
function stoneJoints(sc, sr, h, yb = 0, col = VS.mortar) {
  ...
  for (let i = 1; i < rows; i++) {
    const t = i / rows;
    s += L(lerp(b.left, up(b.left, h), t), lerp(b.front, up(b.front, h), t), col, 1);
    s += L(lerp(b.front, up(b.front, h), t), lerp(b.right, up(b.right, h), t), col, 1);
    // versetzte Stoßfugen
    const lp = lerp(b.left, up(b.left, h), t), fp = lerp(b.front, up(b.front, h), t);
...
function crenels(sc, sr, yb, col, colD) {
  ...
  const merlon = (base, dir) => {
    for (let i = 0; i < 4; i++) {
      if (i % 2) continue;
      const a = lerp(base[0], base[1], i / 4), c2 = lerp(base[0], base[1], (i + 1) / 4);
...
const blob = (x, y, rx, ry, f) => `<ellipse cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" rx="${rx}" ry="${ry}" fill="${f}"/>`;
...
function bRathaus() {
  return (
    ...
    P([[-7, 0], [9, -8], [9, -28], [-7, -20]], VS.woodD) +
    L([-7, -10], [9, -18], VS.woodL, 1.4) + L([1, -4], [1, -24], VS.woodL, 1.4) +
    C(18, -36, 3, VS.win, ' filter="url(#vsGlow)"') + C(-22, -36, 3, VS.win, ' filter="url(#vsGlow)"')
  );
}
...
function bHolz() {
  return (
    ...
    C(46, -4, 6, VS.woodD) + C(58, -4, 6, VS.woodD) + C(52, -13, 6, VS.woodD) +
    C(46, -4, 2.6, VS.woodL) + C(58, -4, 2.6, VS.woodL) + C(52, -13, 2.6, VS.woodL) +
    // Sägebock
    L([28, 4], [40, -12], VS.woodD, 2.4) + L([40, 4], [28, -12], VS.woodD, 2.4) +
    // Tür
    P([[-6, 0], [6, -6], [6, -20], [-6, -14]], VS.woodD)
  );
}
...
function bStein() {
  return (
    ...
    P([[-3, -30], [11, -30], [9, -20], [-1, -20]], VS.stoneL) +
    P([[9, -20], [11, -30], [11, -22]], VS.stoneD)
  );
}

function bEisen() {
  const oreDark = '#3a2e2a', oreMid = '#6b4a3a', oreHi = '#a8674a', iron = '#7d5b4a';
  return (
    // Felssockel + Bergflanke, in die der Stollen führt
    prism(0.5, 0.5, 12, '#8b929f', '#565c68', '#767d8b') +
    P([[-18, -12], [0, -48], [28, -12]], 'url(#vsRock)') +                // Bergflanke
    P([[0, -48], [28, -12], [12, -12]], VS.stoneD) +
    ...
    P([[-16, -12], [4, -24], [4, -6], [-16, 0]], VS.dark) +
    L([-16, -12], [-16, 0], VS.woodD, 3) + L([4, -24], [4, -6], VS.woodD, 3) +
    L([-16, -12], [4, -24], VS.wood, 3) +
    C(-6, -12, 2.4, VS.win, ' filter="url(#vsGlow)"') +
    // Schienen aus dem Stollen nach vorne, mit Schwellen
    L([-8, -2], [-4, 26], VS.stoneD, 2) + L([2, -4], [12, 24], VS.stoneD, 2) +
    `<path d="M-8 4 l10 -1 M-6 12 l12 -1 M-5 20 l14 -1" stroke="${VS.woodD}" stroke-width="1.6"/>` +
    // Lore (Minenwagen) voller Eisenerz
    `<g transform="translate(6,22)">` +
      P([[-11, -6], [11, -6], [8, 3], [-8, 3]], iron) +
      L([-11, -6], [11, -6], VS.woodL, 1.2) +
      C(-8, -8, 3, oreHi) + C(-1, -9, 3.4, oreMid) + C(6, -8, 3, oreHi) + C(1, -6, 3, oreDark) +
      C(-6, 5, 3, VS.dark) + C(6, 5, 3, VS.dark) +
    `</g>` +
    // Erzhaufen daneben (rötlich-metallisch)
    C(-30, 3, 5, oreMid) + C(-22, 5, 5, oreDark) + C(-26, -2, 5, oreHi) + C(-19, 0, 4, oreMid) +
    ...
    C(14, -60, 4, VS.woodL) + C(14, -60, 1.6, VS.dark) +
...
function bLager() {
  return (
    ...
    // großes Holztor
    P([[-10, -2], [12, -13], [12, -32], [-10, -21]], VS.woodD) +
    L([-10, -2], [12, -32], VS.woodL, 1.6) + L([12, -13], [-10, -21], VS.woodL, 1.6) +
    // Fässer davor
    barrel(-30, 4) + barrel(-18, 10) +
...
function bFarm() {
  ...
  return (
    ...
    P([mid([0, 0], b.left), mid([0, 0], b.back), mid([0, 0], b.right), mid([0, 0], b.front)], '#6b7f3a') +
...
function bKaserne() {
  return (
    ...
    // Torbogen
    P([[-7, 0], [9, -8], [9, -30], [-7, -22]], VS.dark) +
    L([-7, -11], [9, -19], VS.woodD, 2) +
    C(-22, -34, 3, VS.win, ' filter="url(#vsGlow)"') + C(24, -34, 3, VS.win, ' filter="url(#vsGlow)"') +
    ...
    C(34, -19, 1.6, VS.stoneL) + C(39, -19, 1.6, VS.stoneL) + C(44, -19, 1.6, VS.stoneL)
  );
}
...
function bMarkt() {
  const stall = (dx, awn, awnL) => {
    ...
    return `<g transform="translate(${dx},0)">` +
      ...
      P(midPts, awnL) +
      // Warenauslage
      C(-7, -2, 2.6, VS.cloth1) + C(0, -3, 2.6, VS.thatchL) + C(7, -2, 2.6, VS.leaf) +
      `</g>`;
  };
  return stall(-20, VS.cloth1, VS.cloth1L) + stall(22, VS.cloth2, VS.cloth2L) + barrel(0, 8);
}
...
function bGate(level) {
  ...
  const tower = (dx) =>
    ...
    (stone ? stoneJoints(0.17, 0.17, 46) + crenels(0.17, 0.17, -46, colL, colD)
  ...
  return (
    ...
    P([[-18, -6], [18, 12], [18, -22], [-18, -40]], colD) +
    P([[-14, -10], [14, 4], [14, -24], [-14, -38]], VS.dark) +
    L([-14, -24], [14, -10], VS.woodD, 1.4) + L([0, -17], [0, -33], VS.woodD, 1.4)
  );
}

const ART = {
  rathaus: bRathaus, holz: bHolz, stein: bStein, eisen: bEisen,
  lager: bLager, farm: bFarm, kaserne: bKaserne, markt: bMarkt,
};
...
function tree(seed = 0) {
  ...
  return (
    ...
    (pine
      ...
      ? P([[-16, -h + 2], [0, -h - 20], [16, -h + 2]], VS.pineD) +
        P([[-13, -h - 8], [0, -h - 28], [13, -h - 8]], VS.pine) +
        P([[-9, -h - 18], [0, -h - 36], [9, -h - 18]], VS.leafL)
...
function rock() {
  return (
    ...
    P([[-4, 8], [16, -38], [34, 8]], 'url(#vsRock)') +
    P([[16, -38], [34, 8], [24, 8]], VS.stoneD) +
    P([[-32, 10], [-12, -28], [10, 10]], 'url(#vsRock)') +
    P([[-12, -28], [10, 10], [0, 10]], VS.stoneD) +
...
function flowers() {
  let s = '';
  const spots = [[-20, 4], [-6, -6], [10, 2], [24, -4], [0, 10], [-14, -12], [16, 12]];
  const cols = ['#e0b84a', '#c25a52', '#e6e0d2', '#7a97bf'];
  spots.forEach((p, i) => { s += C(p[0], p[1], 2.6, cols[i % cols.length]); s += C(p[0], p[1] - 2.6, 1.3, '#fff'); });
  return s;
}
...
function islandSlab() {
  ...
  const soilE = [E[0], E[1] + RIM], soilS = [S[0], S[1] + RIM], soilW = [W[0], W[1] + RIM];
  return (
...
function grassTile(cx, cy, c, r) {
  ...
  let out = P([n, e, s, w], 'url(#vsGrass)', ` stroke="${VS.grassEdge}" stroke-width="0.7" stroke-opacity="0.4"`);
  let seed = (c * 928371 + r * 12345 + 555) >>> 0;
  const rnd = () => ((seed = (seed * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff);
  for (let i = 0; i < 5; i++) {
    const dx = (rnd() - 0.5) * HW * 1.1, dy = (rnd() - 0.5) * HH * 1.1;
...
function cobbleTile(cx, cy, c, r) {
  const n = [cx, cy - HH], e = [cx + HW, cy], s = [cx, cy + HH], w = [cx - HW, cy];
  let out = P([n, e, s, w], 'url(#vsCobble)', ` stroke="${VS.cobbleD}" stroke-width="0.6" stroke-opacity="0.5"`);
  let seed = (c * 40503 + r * 7919 + 13) >>> 0;
  const rnd = () => ((seed = (seed * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff);
  for (let i = 0; i < 10; i++) {
    const dx = (rnd() - 0.5) * HW * 1.2, dy = (rnd() - 0.5) * HH * 1.2;
...
function wallEdge(g1, g2, level) {
  ...
  const s1 = iso(g1[0], g1[1]), s2 = iso(g2[0], g2[1]);
  const face = P([s1, s2, up(s2, h), up(s1, h)], stone ? VS.stoneD : VS.woodD);
  let s = face;
  if (stone) {
    // Fugen
    for (let t = 0.33; t < 1; t += 0.33) s += L(lerp(s1, up(s1, h), t), lerp(s2, up(s2, h), t), VS.mortar, 1);
  }
...
function walls(level, front) {
  if (level < 1) return '';
  const A = [-0.5, -0.5], B = [4.5, -0.5], Cc = [4.5, 4.5], D = [-0.5, 4.5];
  const back = wallEdge(A, B, level) + wallEdge(D, A, level);
  const frontEdges = wallEdge(B, Cc, level) + wallEdge(Cc, D, level);
  return front ? `<g opacity="0.98">${frontEdges}</g>` : `<g opacity="0.9">${back}</g>`;
}
...
function sky() {
  ...
  return (
    ...
    cloud(320, 90, 1, 96) + cloud(1080, 150, 0.8, 128) + cloud(660, 60, 0.6, 160)
  );
}
...
function plot(key, state, meta, selected) {
  ...
  const selRing = selected === key
    ? P([[0, -HH], [HW, 0], [0, HH], [-HW, 0]], 'none', ` class="vs-selring"`)
    : '';
...
function decorObj(c, r) {
  ...
  const [cx, cy] = iso(c, r);
  return { depth: c + r, svg: `<g transform="translate(${cx.toFixed(1)},${cy.toFixed(1)})">${art}</g>` };
}
